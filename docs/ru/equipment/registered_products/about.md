...
Нажмите на строку изделия, чтобы открыть его карточку с паспортом и QR-кодом. Кнопка **Зарегистрировать изделие** запускает мастер регистрации.
...
- **Префикс** – текстовая часть номера перед числом, например `ЗД-`;
- **Стартовый номер** * – первое число серии;
- **Шаг** – шаг между номерами (по умолчанию 1);
- **Суффикс** – текстовая часть после числа, например `-2026`;
- **Количество** * – сколько изделий выпустить (максимум 250 за раз).

Ниже система показывает сводку по номерам: сколько свободно, дублей и занято. Кликните любой номер в списке, чтобы изменить его вручную. **Дата изготовления (поставки)** * указывается одна для всей серии.
...
Дальнейшая работа с выпущенными изделиями (скачивание паспортов и QR-кодов) показана в разделе «Быстрый старт».
